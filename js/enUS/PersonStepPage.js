...
function pageGraphPersonStep(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapPersonStep) {
      window.geoJSONPersonStep.clearLayers();
      $.each( window.listPersonStep, function(index, personStep) {
        if(personStep.location) {
          ...
          shapes.forEach(function(shape, index) {
            ...
            window.geoJSONPersonStep.addLayer(layer);
          });
        }
      });
    } else {
      window.mapPersonStep = L.map('htmBodyGraphLocationPersonStepPage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      window.geoJSONPersonStep = L.geoJSON().addTo(window.mapPersonStep);
      $.each( window.listPersonStep, function(index, personStep) {
        if(personStep.location) {
          var shapes = [];
          if(Array.isArray(personStep.location))
