...
function pageGraphBicycleStep(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapBicycleStep) {
      window.geoJSONBicycleStep.clearLayers();
      $.each( window.listBicycleStep, function(index, bicycleStep) {
        if(bicycleStep.location) {
          ...
          shapes.forEach(function(shape, index) {
            ...
            window.geoJSONBicycleStep.addLayer(layer);
          });
        }
      });
    } else {
      window.mapBicycleStep = L.map('htmBodyGraphLocationBicycleStepPage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      window.geoJSONBicycleStep = L.geoJSON().addTo(window.mapBicycleStep);
      $.each( window.listBicycleStep, function(index, bicycleStep) {
        if(bicycleStep.location) {
          var shapes = [];
          if(Array.isArray(bicycleStep.location))
