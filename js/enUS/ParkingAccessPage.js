...
function pageGraphParkingAccess(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapParkingAccess) {
      window.geoJSONParkingAccess.clearLayers();
      $.each( window.listParkingAccess, function(index, parkingAccess) {
        if(parkingAccess.areaServed) {
          ...
          shapes.forEach(function(shape, index) {
            ...
            window.geoJSONParkingAccess.addLayer(layer);
          });
        }
      });
    } else {
      window.mapParkingAccess = L.map('htmBodyGraphLocationParkingAccessPage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      window.geoJSONParkingAccess = L.geoJSON().addTo(window.mapParkingAccess);
      $.each( window.listParkingAccess, function(index, parkingAccess) {
        if(parkingAccess.areaServed) {
          var shapes = [];
          if(Array.isArray(parkingAccess.areaServed))
